...
public:
        ...
        cudaStream_t  stream)
    {
        ...
        // Pass 0: histogram-only over current batch indices
        ...
        cudaMemsetAsync(d_migration_count_, 0, sizeof(uint32_t), stream);

        // Pass 1: classify + scatter
        ...
        ClassifyAndScatterKernel<float, 256>
            ...
            th,
            d_tier_bufs_[0],  // H100
            d_tier_bufs_[1],  // A6000
            d_tier_bufs_[2],  // CPU-staged
            d_output,
            d_migration_queue_,
            d_migration_count_);
    ...
    void process_migrations(cudaStream_t stream)
    {
        ...
        cudaMemcpyAsync(&queue_len, d_migration_count_, sizeof(uint32_t),
                        cudaMemcpyDeviceToHost, stream);
        cudaStreamSynchronize(stream);

        if (queue_len == 0) return;

        // Cap to migration budget (prevents PCIe stalls during training)
    ...
    std::pair<float, float> verify_drift(cudaStream_t stream)
    {
        ...
        float* d_abs = nullptr;
        float* d_rel = nullptr;
        cudaMalloc(&d_abs, sizeof(float));
        cudaMalloc(&d_rel, sizeof(float));
        cudaMemsetAsync(d_abs, 0, sizeof(float), stream);
        cudaMemsetAsync(d_rel, 0, sizeof(float), stream);
        ...
        cudaMemcpyAsync(&h_abs, d_abs, sizeof(float), cudaMemcpyDeviceToHost, stream);
        cudaMemcpyAsync(&h_rel, d_rel, sizeof(float), cudaMemcpyDeviceToHost, stream);
        cudaStreamSynchronize(stream);
...
private:
    ...
    // Migration queue (device memory)
    MigrationEntry* d_migration_queue_ = nullptr;
    uint32_t*       d_migration_count_ = nullptr;
    ...
    void init_buffers()
    {
        ...
        cudaMalloc(&d_migration_queue_, N * sizeof(MigrationEntry));
        cudaMalloc(&d_migration_count_, sizeof(uint32_t));

        cudaMemset(d_freq_hist_, 0, N * sizeof(uint32_t));
        // Initialize all rows to CPU tier
        cudaMemset(d_row_tier_, static_cast<int>(Tier::CPU_DRAM), N);

        // Tier buffers — in production these live on different devices
        for (int t = 0; t < 3; ++t)
        {
    ...
    void free_buffers()
    {
        ...
        cudaFree(d_migration_queue_);
        cudaFree(d_migration_count_);
        for (int t = 0; t < 3; ++t)
        {
